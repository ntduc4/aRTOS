...
//
// TODO: Add different scheduling algo / priority support later
static rtos_tcb_t *
rtos_scheduler_select_next(rtos_stack_word_t *current_stack_pointer) {
  ...
  if (current_stack_pointer == NULL) {
    ...
    _cur_task = _ready_l.sentinel.next->owner;
    rtos_list_remove(&_cur_task->state_item);
    return _cur_task;
  }
  ...
  if (_cur_task != &idle_task && _cur_task->state_item.container == NULL)
    rtos_list_insert_end(&_ready_l, &_cur_task->state_item);
