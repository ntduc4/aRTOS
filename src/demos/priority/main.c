...
static rtos_stack_word_t worker_a_stack[TASK_STACK_WORDS]
    __attribute__((aligned(8)));
static rtos_stack_word_t worker_b_stack[TASK_STACK_WORDS]
    __attribute__((aligned(8)));
...
static void uart_lock_take(void) {
  (void)rtos_semaphore_take(uart_lock, RTOS_DELAY_INFINITY);
}

static void high_task(void *argument) {
  (void)argument;
  uint32_t release_tick = rtos_get_tick();
...
static void create_task_or_halt(rtos_task_fn_t entry, void *argument,
  ...
  if (rtos_task_create(entry, argument, stack, TASK_STACK_WORDS, priority) !=
      RTOS_OK) {
    for (;;) {
    }
  }
...
int main(void) {
  ...
  demo_uart_write_string(
      "Two priority-0 tasks round-robin; priority 1 preempts them every second.\n");
